...
export function SpeakerPanel() {
  ...
  const setSpeakerOutlineColor = useCaptionStore(
    (s) => s.setSpeakerOutlineColor,
  );
  const setSpeakerOutlineThickness = useCaptionStore(
    (s) => s.setSpeakerOutlineThickness,
  );
  ...
  return (
    <div className="bg-card border-border mb-3 overflow-visible rounded-md border">
      <div className="border-border flex items-center justify-between border-b px-4 py-3">
        <span className="text-accent-light text-xs font-semibold tracking-[0.08em] uppercase">
          Speakers
        </span>
        <span className="text-text-dim text-[11px]">{speakers.length}</span>
      </div>
      <div className="flex flex-col py-2">
        {speakers.map((label) => {
          const textColor = speakerColors[label] ?? captionStyle.color;
          const outlineColor =
            speakerOutlineColors[label] ?? captionStyle.outlineColor;
          const thickness =
            speakerOutlineThickness[label] ?? captionStyle.outlineThickness;
          const fontFamily =
            speakerFontFamilies[label] ?? captionStyle.fontFamily;
        ...
        {adding ? (
          <div className="flex items-center gap-2.5 px-4 py-2">
            <input
              ...
              className="bg-input border-accent text-text-primary flex-1 rounded-md border px-2.5 py-1.5 font-mono text-[13px]"
            />
          </div>
        ) : (
          <button
            onClick={() => setAdding(true)}
            className="border-border text-text-muted hover:border-accent-light hover:text-accent-light mx-3 mt-1.5 mb-1 cursor-pointer rounded-md border border-dashed bg-transparent px-2.5 py-1.5 text-left text-xs transition-colors"
          >
            + Add speaker
          </button>
        )}
      ...
      <DeleteSpeakerModal
        ...
        captionCount={
          deleteTarget ? (captionCountsBySpeaker[deleteTarget] ?? 0) : 0
        }
        otherSpeakers={
          deleteTarget ? speakers.filter((s) => s !== deleteTarget) : []
        }
