...
function buildPalette(labels: string[]): Record<string, string> {
  ...
  labels.forEach((label, i) => {
    out[label] = SPEAKER_PALETTE[i % SPEAKER_PALETTE.length];
  });
  return out;
}
...
function buildOutlinePalette(labels: string[]): Record<string, string> {
  ...
  labels.forEach((label) => {
    out[label] = DEFAULT_SPEAKER_OUTLINE;
  });
  return out;
}
...
function patchDiffers(prev: Caption, patch: CaptionPatch): boolean {
  ...
  if ("color_override" in patch && patch.color_override !== prev.color_override)
    return true;
  if (
    "outline_override" in patch &&
    patch.outline_override !== prev.outline_override
  )
    return true;
  if (
    "outline_thickness" in patch &&
    patch.outline_thickness !== prev.outline_thickness
  )
    return true;
  if ("font_family" in patch && patch.font_family !== prev.font_family)
    return true;
  if ("font_size" in patch && patch.font_size !== prev.font_size) return true;
  if ("pos_x_override" in patch && patch.pos_x_override !== prev.pos_x_override)
    return true;
  if ("pos_y_override" in patch && patch.pos_y_override !== prev.pos_y_override)
    return true;
  if ("align_override" in patch && patch.align_override !== prev.align_override)
    return true;
  return false;
}
...
export const useCaptionStore = create<CaptionStore>((set) => ({
  ...
  setSpeakerColor: (label, color) =>
    set((store) => ({
      speakerColors: { ...store.speakerColors, [label]: color },
    })),
  ...
  setSpeakerAlign: (label, value) =>
    set((store) => ({
      speakerAlign: { ...store.speakerAlign, [label]: value },
    })),
  addSpeaker: (label) =>
    set((store) => {
      ...
      const color =
        SPEAKER_PALETTE[(nextSpeakers.length - 1) % SPEAKER_PALETTE.length];
      return {
  ...
  renameSpeaker: (oldLabel, newLabel) =>
    set((store) => {
      ...
      if (
        !store.speakers.includes(oldLabel) ||
        store.speakers.includes(trimmed)
      )
        return store;

      // Preserve ordering in the speakers list.
      const speakers = store.speakers.map((s) =>
  ...
  setCurrentTime: (t) =>
    set((s) => (s.currentTime === t ? s : { currentTime: t })),
  setVideoDuration: (d) =>
    set((s) => (s.videoDuration === d ? s : { videoDuration: d })),
  requestSeek: (t) =>
    set((s) => (s.seekRequest === t ? s : { seekRequest: t })),
  requestScrollToCaption: (id) =>
    set((s) =>
      s.scrollToCaptionRequest === id ? s : { scrollToCaptionRequest: id },
    ),
  setError: (msg) => set((s) => (s.error === msg ? s : { error: msg })),
  setTranscribeProgress: (p) =>
    set((s) => (s.transcribeProgress === p ? s : { transcribeProgress: p })),
  setTranscribeStage: (st) =>
    set((s) => (s.transcribeStage === st ? s : { transcribeStage: st })),
  setTranscribeConfig: (patch) =>
    set((store) => ({
      transcribeConfig: { ...store.transcribeConfig, ...patch },
    })),
  ...
  setCaptionStyle: (patch) =>
    set((store) => ({ captionStyle: { ...store.captionStyle, ...patch } })),
  setReTranscribing: (v) => set({ isReTranscribing: v }),
  setThumbnail: (dataUrl) => set({ thumbnail: dataUrl }),
  setProjectName: (name) =>
    set({ projectName: name && name.trim() ? name.trim() : null }),
  loadSavedSession: (data) =>
    set({
