...
export function buildCaptionautFile(input: Omit<CaptionautImport, never> & { sourceFileName: string | null }): CaptionautFile {
  return {
...
export function parseCaptionautFile(raw: string): CaptionautImport {
  ...
  if (typeof parsed.version !== 'number' || parsed.version > CAPTIONAUT_VERSION) {
    throw new Error(`Unsupported Captionaut file version (${String(parsed.version)}).`)
  }
